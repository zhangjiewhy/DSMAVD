    ...
    def __call__(self, model: nn.Module) -> OptimWrapper:
        ...
        for v in model.modules():
            ...
            if isinstance(v, nn.modules.batchnorm._NormBase):
                ...
                params_groups[0].append(v.weight)
        for v in model.modules():    #my  add
            if hasattr(v, 'relative_position_bias_table') and isinstance(v.relative_position_bias_table, nn.Parameter):
                print('add:'+'relative_position_bias_table')
                params_groups[2].append(v.relative_position_bias_table)
            if hasattr(v, 'bias_avg') and isinstance(v.bias_avg, nn.Parameter):   #my add
                params_groups[0].append(v.beta_rgb)     #my add
