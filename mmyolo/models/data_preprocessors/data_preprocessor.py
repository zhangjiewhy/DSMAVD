...
class YOLOv5DetDataPreprocessor(DetDataPreprocessor):
    ...
    def __init__(self, *args, non_blocking: Optional[bool] = True, **kwargs):
        ...
        self.mean_RGBT = self.mean.repeat(2, 1, 1)  # my add
        self.std_RGBT = self.std.repeat(2, 1, 1)
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        """
        if not training:
            # return super().forward(data, training)   #原有
            return self.forward_test(data, training)
        

        data = self.cast_data(data)  #复制数据去设备
        inputs, data_samples = data['inputs'], data['data_samples']
        assert isinstance(data['data_samples'], dict)

        # TODO: Supports multi-scale training
        if  inputs.shape[1] == 6:                                                        #my add                #my add
            if self._channel_conversion and inputs.shape[1] == 6:                        #my add
                inputs = inputs[:, [2, 1, 0, 5, 4, 3], ...]                               #my add
            if self._enable_normalize:                                                     #my add
                inputs = (inputs - self.mean.repeat(2, 1, 1)) / self.std.repeat(2, 1, 1)              #my add
        else:                                                         #my add
            if self._channel_conversion and inputs.shape[1] == 3:
                inputs = inputs[:, [2, 1, 0], ...]                #BGR 转 RGB
    ...
    def forward_test(self, data: dict, training: bool = False) -> dict:
        ...
        if is_seq_of(_batch_inputs, torch.Tensor):
            ...
            for _batch_input in _batch_inputs:
                if _batch_input.shape[0] == 6:  # below if is  my add
                    # channel transform
                    if self._channel_conversion:
                        _batch_input = _batch_input[[2, 1, 0, 5, 4, 3], ...]
                        _batch_input = _batch_input.float()
                        # Normalization.
                    if self._enable_normalize:
                        assert _batch_input.dim() == 3 and _batch_input.shape[0] == 6, (
                            'If the mean has 6 values, the input tensor '
                            'should in shape of (6, H, W), but got the tensor '
                            f'with shape {_batch_input.shape}')
                        _batch_input = (_batch_input - self.mean.repeat(2, 1, 1)) / self.std.repeat(2, 1, 1)
                else:  # above if is  my add
                    # channel transform
                    if self._channel_conversion:
