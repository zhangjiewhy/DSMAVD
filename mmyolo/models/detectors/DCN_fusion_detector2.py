...
class DCN_Fus_Detector2(SingleStageDetector):
    ...
    def __init__(self,
        ...
        # self.fusion_method = self.ADD_BN()
    ...
    def predict(self,
        ...
        x = self.extract_feat(batch_inputs)
        # results_list,head_feature = self.bbox_head.predict(
        #     x, batch_data_samples, rescale=rescale)  #head_feature调试可视化特征图用
        ...
        if model == 'add':
            if self.fusion_location == 'before_backbone':
                ...
                for i in range(len(x)):
                    ...
                    x_fus.append(x[i] + x_i[i])                          #直接相加
                    # x_fus.append(self.fusion_method[i](x[i] + x_i[i]))    #BN+SILU
        if model == 'Concat':
            if self.fusion_location == 'before_backbone':
