...
class YOLOv8_backbone_dcn(BaseBackbone):
    ...
    def DCN_align(self):
        ...
        inchannel = [128, 256, 512]   #i-1
        # inchannel = [256, 512, 1024]  #i-2
    ...
    def forward(self, x: torch.Tensor,x_i: torch.Tensor) -> tuple:
        ...
        for i, layer_name in enumerate(self.layers):
            ...
            if i in self.out_indices:
                outs.append(x)
                # x_i = self.align_method[i-2](x, x_i)
                outs_i.append(x_i)
            if i > 0 and i < 4:
                x_i = self.align_method[i-1](x, x_i)
