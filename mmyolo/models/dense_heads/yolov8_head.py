...
class YOLOv8HeadModule(BaseModule):
    ...
    def forward_single(self, x: torch.Tensor, cls_pred: nn.ModuleList,
        ...
        b, _, h, w = x.shape
        cls_logit = cls_pred(x)   #(b,num_class,h,w)
        ...
        if self.reg_max > 1:
            ...
            bbox_preds = bbox_preds.transpose(1, 2).reshape(b, -1, h, w)   #(b,4，h，w)
        else:
    ...
    def loss_by_feat(
        ...
        if current_featmap_sizes != self.featmap_sizes_train:
            ...
            mlvl_priors_with_stride = self.prior_generator.grid_priors(
                ...
                with_stride=True)  #得到每个特征图特征点在输入图像上的坐标
        ...
        gt_labels = gt_info[:, :, :1]   #类别标签
        gt_bboxes = gt_info[:, :, 1:]  # xyxy    边框标签
        ...
        assigned_result = self.assigner(
            ...
            gt_labels, gt_bboxes, pad_bbox_flag)    #制作标签，用于后续损失计算
        ...
        loss_cls = self.loss_cls(flatten_cls_preds, assigned_scores).sum()      #flatten_cls_preds和assigned_scores为（b，锚点数，类别数）
