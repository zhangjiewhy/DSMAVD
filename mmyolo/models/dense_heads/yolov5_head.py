...
class YOLOv5Head(BaseDenseHead):
    ...
    def predict_by_feat(self,
                        ...
                        rescale: bool = True,
                        with_nms: bool = True) -> List[InstanceData]:     # masks=None
        ...
        # a2cls_scores_cpu=[]
        # for i in range(3):
        #     a2cls_scores_cpu.append(cls_scores[i].sigmoid().cpu().numpy())
        ...
        flatten_cls_scores = torch.cat(flatten_cls_scores, dim=1).sigmoid()   #分类得分使用sigmoid（）激活

        # maskss = []
        # a1maskss_cpu = []
        # masks_i = masks[0]
        # masks_rgb = masks[1]
        # for i in range(len(masks_i)):
        #     masks_v = masks_rgb[i].sigmoid()
        #     masks_t = masks_i[i].sigmoid()
        #     maskss.append(torch.maximum(masks_v, masks_t))
        #     a1maskss_cpu.append(maskss[i].cpu().numpy())
        # flatten_masks = [
        #     mask.permute(0, 2, 3, 1).reshape(num_imgs, -1, self.num_classes)
        #     for mask in maskss
        # ]
        # flatten_masks = torch.cat(flatten_masks, dim=1)
        # B, N, C = flatten_cls_scores.size()
        # for b in range(B):
        #     for n in range(N):
        #         if flatten_masks[b, n, :] - flatten_cls_scores[b, n, :] > 0.3:
        #             aa = flatten_masks[b, n, :]
        #             ab = flatten_cls_scores[b, n, :]
        #             # flatten_cls_scores[b, n, :]=torch.maximum(flatten_cls_scores[b, n, :], flatten_cls_scores[b, n, :])
        #             flatten_cls_scores[b, n, :] = torch.maximum(flatten_cls_scores[b, n, :], flatten_masks[b, n, :])
    ...
    def loss(self, x: Tuple[Tensor], batch_data_samples: Union[list, dict],ProbEN_positive=None) -> dict:
        ...
        if isinstance(batch_data_samples, list):
            ...
            outs = self(x)       # 这里进入head的forward（），再进入head_mould的forward（）
            #  outs【类别预测(b,类别数，h，w) ，边框预测（b,4，h，w），dfl预测（b,h*w，4，16）
            #得到的cls_pred   list:3   tensor(b,num_class,h,w)
            # Fast version
            loss_inputs = outs + (batch_data_samples['bboxes_labels'],
                                  batch_data_samples['img_metas'])
            losses = self.loss_by_feat(*loss_inputs) #v8进入v8_head的loss_by_feat（）
