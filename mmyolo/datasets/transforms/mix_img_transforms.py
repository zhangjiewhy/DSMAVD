    ...
    def transform(self, results: dict) -> dict:
        ...
        for _ in range(self.max_refetch):
            ...
            if self.use_cached:
                ...
                # get images information will be used for Mosaic or MixUp   #得到多张图片的信息
                mix_results = [
        ...
        # Mosaic or MixUp
        results = self.mix_img_transform(results)    #上面是选择参与Mosaic和MixUp的图片，并存储于results['mix_results']中
    ...
    def mix_img_transform(self, results: dict) -> dict:
        ...
        if len(results['img'].shape) == 3:
            ...
            if results['img'].shape[2]==6:
                mosaic_img = np.full(
                    (int(img_scale_h * 2), int(img_scale_w * 2), 6),
                    self.pad_val,
                    dtype=results['img'].dtype)
        else:
