    ...
    def _resize_img(self, results: dict):
        ...
        if results.get('img', None) is not None:
            ...
            if ratio != 1:
                ...
                if image.shape[2]==6:                                    #add
                    image_rgb = mmcv.imrescale(
                        img=image[:,:,0:3],
                        scale=ratio,
                        interpolation='area' if ratio < 1 else 'bilinear',
                        backend=self.backend)          #add
                    image_i = mmcv.imrescale(
                        img=image[:, :, 3:],
                        scale=ratio,
                        interpolation='area' if ratio < 1 else 'bilinear',
                        backend=self.backend)                           #add
                    image=np.concatenate((image_rgb,image_i), axis=2)   #add
                else:                                                      #add
...
class LetterResize(MMDET_Resize):
    ...
    def _resize_img(self, results: dict):
        ...
        if top_padding != 0 or bottom_padding != 0 or \
            ...
            if isinstance(pad_val, tuple):  #my  add
                pad_val_rgb = pad_val[0:3]
                pad_val_i = pad_val[3:]
            else:                                                        #my  add
                pad_val_rgb=pad_val
                pad_val_i=pad_val
            if  image.shape[2]==6:                                     #my  add
                image_rgb = mmcv.impad(
                    img=image[:,:,0:3],
                    padding=(padding_list[2], padding_list[0], padding_list[3],
                             padding_list[1]),
                    pad_val=pad_val_rgb,
                    padding_mode='constant')                        #my  add
                image_i = mmcv.impad(
                    img=image[:, :, 3:],
                    padding=(padding_list[2], padding_list[0], padding_list[3],
                             padding_list[1]),
                    pad_val=pad_val_i,
                    padding_mode='constant')                          #my  add
                image=np.concatenate((image_rgb,image_i), axis=2)  #my  add
            else:                                                   #my  add
                image = mmcv.impad(
                    img=image,
                    padding=(padding_list[2], padding_list[0], padding_list[3],
...
class YOLOv5HSVRandomAug(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if results['img'].shape[2]==6:                    #my add
            results['img_hsvi']=results['img'][:, :, 3:6]   #my add
            results['img'] = results['img'][:, :, 0:3]   #my add
...
####################add
        if 'img_hsvi' in results:
            hue, sat, val = cv2.split(
                cv2.cvtColor(results['img_hsvi'], cv2.COLOR_BGR2HSV))
            ...
            im_hsv = cv2.merge(
                (cv2.LUT(hue, lut_hue), cv2.LUT(sat,
                                                lut_sat), cv2.LUT(val, lut_val)))
            results['img_hsvi'] = cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR)
            results['img'] =np.concatenate(( results['img'],  results['img_hsvi']), axis=2)
#############add
        return results
...
class RegularizeRotatedBox(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
     ...
     #add   RGBT
@TRANSFORMS.register_module()
class LoadImageFromFile_RGBT(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if '+' in filename:
            filename_i, filename = filename.split('+')
            results['data_prefix']=dict(img_i=filename_i)
        ...
        if  'img_i' in results['data_prefix']:
            filename_i = os.path.join(results['data_prefix']['img_i'], filename.split('/')[-1])
            file_client_i = fileio.FileClient.infer_client(self.file_client_args, filename_i)
            img_bytes_i = file_client_i.get(filename_i)
            img_i = mmcv.imfrombytes(img_bytes_i, flag=self.color_type, backend=self.imdecode_backend)
            if self.to_float32:
